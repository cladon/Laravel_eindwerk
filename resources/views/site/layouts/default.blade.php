            ...
            <div class="collapse navbar-collapse navbar-ex1-collapse">
                <ul class="nav navbar-nav">
                    <li {{ (Request::is('/') ? ' class="active"' : '') }}><a
                                href="{{{ URL::to('') }}}">{{{ trans('site/site.welcome') }}}</a></li>
                </ul>
...
//donald
                    ...
                    @if (Auth::check())
                        ...
                        @else
                            <li><a href="{{{ URL::to('/jtable') }}}">{{{trans('site/site.admin_panel') }}}</a></li>
                        @endif
                        <li><a href="#">{{{ trans('site/site.login_as') }}} {{{Auth::user()->name }}}</a></li>
                        <li><a href="{{{ URL::to('auth/logout') }}}">{{{trans('site/site.logout') }}}</a></li>
                    @else
                       <li {{ (Request::is('auth/login') ? ' class="active"' : '') }}><a href="{{{ URL::to('auth/login') }}}">{{{trans('site/site.login') }}}</a></li>
